...
async function getModel(providerID: ProviderID, modelID: ModelID, ctx?: InstanceContext) {
  ...
  if (!ctx) return AppRuntime.runPromise(effect)
...
async function drain(input: LLM.StreamInput, ctx?: InstanceContext) {
  if (!ctx) return llm.runPromise((svc) => svc.stream(input).pipe(Stream.runDrain))
...
async function drainWith(layer: Layer.Layer<LLM.Service>, input: LLM.StreamInput, ctx?: InstanceContext) {
  const effect = attach(LLM.Service.use((svc) => svc.stream(input).pipe(Stream.runDrain))).pipe(Effect.provide(layer))
  if (!ctx) return Effect.runPromise(effect)
  return Effect.runPromise(effect.pipe(Effect.provideService(InstanceRef, ctx)))
}
